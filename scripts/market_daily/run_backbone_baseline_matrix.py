...
DEFAULT_MODELS = ("PatchTST", "iTransformer", "TimeMixer", "TimesNet", "WPMixer")
...
MODEL_CONFIGS = {
    "PatchTST": {"d_model": 128, "d_ff": 256, "e_layers": 2, "n_heads": 4, "epochs": 2},
    "iTransformer": {"d_model": 128, "d_ff": 256, "e_layers": 2, "n_heads": 4, "epochs": 2},
    "TimeMixer": {"d_model": 128, "d_ff": 256, "e_layers": 2, "n_heads": 4, "epochs": 3},
    "TimesNet": {"d_model": 128, "d_ff": 256, "e_layers": 2, "n_heads": 4, "epochs": 2},
    "WPMixer": {"d_model": 128, "d_ff": 256, "e_layers": 2, "n_heads": 4, "epochs": 2},
}
...
def build_setting(year, model, feature_set, seq_len, d_model, d_ff, e_layers, n_heads, embed, distil, des, model_id):
    return (
...
def build_prediction_path(year, model, feature_set, seq_len, d_model, d_ff, e_layers, n_heads, embed, distil, des, model_id):
    return ROOT / "test_results" / build_setting(
        year, model, feature_set, seq_len, d_model, d_ff, e_layers, n_heads, embed, distil, des, model_id
    ) / "top1_predictions.csv"


def build_metrics_path(year, model, feature_set, seq_len, d_model, d_ff, e_layers, n_heads, embed, distil, des, model_id):
    return ROOT / "test_results" / build_setting(
        year, model, feature_set, seq_len, d_model, d_ff, e_layers, n_heads, embed, distil, des, model_id
    ) / "market_metrics.txt"


def build_job_record(year, model, feature_set, seq_len, batch_size, cache_path, learning_rate, loss, huber_delta, des):
    model_cfg = MODEL_CONFIGS[model]
    embed = "fixed" if model in {"TimesNet", "TimeMixer"} else "timeF"
    distil = "True"
    model_id = f"market_{year}_{seq_len}_fs{feature_set}"
    command = [
        sys.executable,
        "run.py",
    ...
    return {
        ...
        "des": des,
        "model_id": model_id,
        "setting": build_setting(year, model, feature_set, seq_len, model_cfg["d_model"], model_cfg["d_ff"], model_cfg["e_layers"], model_cfg["n_heads"], embed, distil, des, model_id),
        "prediction_path": str(build_prediction_path(year, model, feature_set, seq_len, model_cfg["d_model"], model_cfg["d_ff"], model_cfg["e_layers"], model_cfg["n_heads"], embed, distil, des, model_id)),
        "metrics_path": str(build_metrics_path(year, model, feature_set, seq_len, model_cfg["d_model"], model_cfg["d_ff"], model_cfg["e_layers"], model_cfg["n_heads"], embed, distil, des, model_id)),
...
def build_manifest(years, models, feature_set, seq_len, batch_size, cache_path, learning_rate, loss, huber_delta, des):
    rows = []
...
def launch_jobs(output_dir, manifest, gpus, dry_run=False, rerun_completed=False, poll_seconds=30):
    ...
    if dry_run:
        for row in pending_rows:
            print(json.dumps({"year": row["year"], "model": row["model"], "command": json.loads(row["command"])}, ensure_ascii=False))
        return

    active = {}
    queue = list(pending_rows)
...
def summarize_jobs(output_dir, manifest):
    ...
    summary = pd.DataFrame(rows).sort_values(["year", "top1_mean", "top1_sharpe"], ascending=[True, False, False]).reset_index(drop=True)
...
def build_parser():
    parser = argparse.ArgumentParser(description="Run backbone market baselines")
    subparsers = parser.add_subparsers(dest="cmd", required=True)

    manifest = subparsers.add_parser("manifest")
    manifest.add_argument("--output_dir", default="logs/backbone_baseline_matrix")
    manifest.add_argument("--years", default="2021,2022,2023,2024")
    manifest.add_argument("--models", default="PatchTST,iTransformer,TimeMixer,TimesNet,WPMixer")
    ...
    manifest.add_argument("--loss", default="Huber")
    manifest.add_argument("--huber_delta", type=float, default=1.0)
    manifest.add_argument("--des", default="market_backbone_baseline")

    launch = subparsers.add_parser("launch")
    launch.add_argument("--output_dir", default="logs/backbone_baseline_matrix")
    launch.add_argument("--years", default="2021,2022,2023,2024")
    launch.add_argument("--models", default="PatchTST,iTransformer,TimeMixer,TimesNet,WPMixer")
    ...
    launch.add_argument("--loss", default="Huber")
    launch.add_argument("--huber_delta", type=float, default=1.0)
    launch.add_argument("--des", default="market_backbone_baseline")
    ...
    summarize.add_argument("--output_dir", default="logs/backbone_baseline_matrix")
